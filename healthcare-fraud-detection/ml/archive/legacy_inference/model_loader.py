...
def load_artifacts(model_path=None, preprocessor_path=None, metadata_path=None):
    ...
    if _metadata is None:
        if meta_path.exists():
            try:
                ...
                print(f"[ML LOADER] Warning: Failed to parse model metadata: {e}")
                _metadata = {}
        else:
            _metadata = {}
    ...
    if not m_path.exists():
        raise FileNotFoundError(f"[ML LOADER] Keras model file not found at: {m_path}")
    if not p_path.exists():
        raise FileNotFoundError(f"[ML LOADER] Keras preprocessor file not found at: {p_path}")

    if _model is None:
        print(f"[ML LOADER] Loading Keras Deep Learning Model (PyTorch Backend) from {m_path}...")
        _model = keras.models.load_model(str(m_path))
        print("[ML LOADER] Keras model loaded successfully.")

    if _preprocessor is None:
        print(f"[ML LOADER] Loading Preprocessor from {p_path}...")
        _preprocessor = joblib.load(p_path)
        print("[ML LOADER] Preprocessor loaded successfully.")
